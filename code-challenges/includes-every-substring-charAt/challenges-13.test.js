...
const standardizePhoneNumbers = (arr) => {
    let newArr = [];
    arr.forEach(str => {
        newArr.push(str.replace("(", '').replace( ") ", '').replace( "-", '' ))
    })
    return newArr;
};
...
const onlyOddChars = (str) => {
    return(`${str.substring(1,2)}${str.substring(3,4)}${str.substring(5,6)}${str.substring(7,8)}${str.substring(9,10)}${str.substring(11,12)}`)
};
